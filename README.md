...
本实验选择的弹幕和评论区信息特征视频网站实证对象为`BiliBili`平台，它是较早使用弹幕功能的视频在线播放平台之一，具有用户即时发送弹幕、弹幕点赞、弹幕字体颜色，评论互动，评论点赞，热门评论等功能。



### 研究的背景：

随着`BiliBili`弹幕功能的推出，视频的交互性和及时沟通性显著得以提高，视频的距离感更加拉近，弹幕的受众和影响力不断扩大，弹幕和评论信息数量也呈指数级增长。这同时也带来了大量无用信息的弹幕和评论，弹幕和评论的信息有用性检测变得重要，这也能凸显信息的有用性检测的学术价值和商业价值。



### **意义：**

本课程实验主要进行三个方面实际操作：一是探索用户自身特征与信息有用性关系。二是探索文本特征，包括文本长度等。三是探索信息特征，我将弹幕关键词作为主要衡量指标。本文将检测弹幕信息有用性作为目标，从用户特征、表达形式和弹幕和评论的信息效用特征三个角度得到各等级信息有用性的检测结果，概括相关特征作为具有决定性的影响变量。对弹幕文本内容进行挖掘与审核，能正确引导视频观看用户合理讨论，发送文明弹幕，有助于营造和谐稳定的视频观看环境。
...
用户发送会员等级，回复度和点赞数关系如上方表格所示。个体用户特征里的高级会员，UP主能够有更多的效果或文字。且会员等级反映了对视频平台的时间或金钱投入，会员等级高的用户具有更强的依赖性。因等级的升级需要花费大量时间和用户活跃度，所以达到六级是较为难的事。不同的会员等级的用户，其弹幕信息使用经验和倾向可能有所差异，对弹幕信息有用性有一定影响。
...
《舌尖上的中国第一季第七集》分析收集视频评论区数据发现，字符数小于20的有：占比多于50%，大部分评论文本长度为10个字左右，可见一半的用户倾向于即时发表感受或想法，不到`1/10`的用户会考虑根据热点主题进行针对性长篇评论。
...
从传播学角度研究，网络媒体有丰富的符号化表达方式，评论大致可分为文字类和非文字类符号，《巅峰时期的周杰伦有多恐怖》收集分析视频评论区数据后，发现关键词大部分为赞扬感情，而《舌尖上的中国第一季第七集》分析收集视频评论区数据发现大部分表现为观众当时的观看心情。
...
对于弹幕表现形式而言，文本字数占据重要贡献度，其他带有感情的特征项无法做到有用性检测。网络环境中用户更注重信息内容，对于信息表达的规范性和完整度不太关注。

弹幕具有丰富的表现形式，改变了用户的视觉体验，如红色弹幕比较具有警示意义。

相关性有助于探索新的数据集。通过使用`seaborn`的热图，在很短时间内，就可以看到输入数据的相关性，并得到想法来探索，很容易看到最强的相关性在哪里。

本实验尚有许多值得改进的地方，例如在考虑影响因素时，未能涵盖文本语言上的特征。在全面性上也有进一步提高的空间，算法分类精度效果有限，时间允许情况下应使用机器学习算法进行模型训练，对情感文本与含义进行更深层次挖掘，进行深度分析。如果打算使用这些数据来建立一个模型，应该再将其分解或者对其进行随机化。

对弹幕和评论文本内容进行挖掘与审核，能正确引导视频观看用户合理讨论，避免弹幕环境恶化。发送文明弹幕，有助于营造和谐稳定的视频观看环境。
